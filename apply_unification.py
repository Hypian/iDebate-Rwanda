...
# Complete script to update each of the 7 files with unified header, navigation, footer and links.

PAGES_CONFIG = [
    {
        'file': 'index.html',
        'active_nav': 'home',
        'header_start_regex': r'<!-- Top Announcement Strip.*?</header>',
        'footer_start_regex': r'<!-- Monumental Footer.*?>|<!-- MONUMENTAL EDITORIAL FOOTER.*?>|<footer.*?</aside>', # fallback
    },
    {
        'file': 'about.html',
        'active_nav': 'about',
    ...
    {
        'file': 'debate-league.html',
        'active_nav': 'programs',
    },
    {
        'file': 'teachers-debate-institute.html',
        'active_nav': 'programs',
    },
    {
        'file': 'international-program.html',
        'active_nav': 'programs',
    },
    {
...
<header class="w-full border-b border-border-hairline bg-paper-canvas dark:bg-forest-deep sticky top-0 z-50">
  <div class="max-w-7xl mx-auto px-6 lg:px-16 h-20 lg:h-24 flex items-center justify-between">
    ...
    <div class="flex items-center gap-3 lg:gap-4">
      ...
      <button type="button" id="mobile-menu-toggle" aria-label="Toggle Mobile Menu" class="md:hidden p-2 text-on-surface hover:text-primary focus:outline-none bg-transparent border-0 cursor-pointer">
...
def process_file(cfg):
    ...
    # 1. Replace header section (from top of body to <main or <section)
    # The header starts right after <body ...> up to <main or the first <section
    ...
    # Find where main content begins: either <main or <section
    main_start = re.search(r'(<main[^>]*>|<section[^>]*>)', content)
    if not main_start:
        print(f"Failed to find main or section in {filename}")
        return
    
    pre_body = content[:body_match.end()]
    post_header = content[main_start.start():]
    
    new_header = make_header(active_nav)
    
    # 2. In post_header, find the <footer>...</footer> and replace with new footer
    footer_match = re.search(r'<footer.*?</footer>', post_header, re.DOTALL)
    if not footer_match:
        print(f"Failed to find footer in {filename}")
        return
    
    main_content_part = post_header[:footer_match.start()]
    after_footer_part = post_header[footer_match.end():]
    # Remove any stray trailing script or closing body tags in after_footer_part that might duplicate
    after_footer_part = re.sub(r'</body>\s*</html>', '', after_footer_part)
    
    new_footer = make_footer()
    
    assembled = pre_body + "\n" + new_header + "\n" + main_content_part + "\n" + new_footer + "\n</body>\n</html>"
    
    # Specific internal links fix for index.html / about.html
    if filename in ['index.html', 'about.html']:
        # Update pillar cards href
        assembled = assembled.replace('href="#"\n><span', 'href="debate-league.html"\n><span')
        # Replace pillar 01 link
        assembled = re.sub(
            r'(PILLAR 01.*?<a [^>]*href=")#(")',
            r'\1international-program.html\2',
            assembled,
            flags=re.DOTALL
        )
        # Replace pillar 02 link
        assembled = re.sub(
            r'(PILLAR 02.*?<a [^>]*href=")#(")',
            r'\1debate-league.html\2',
            assembled,
            flags=re.DOTALL
        )
        # Replace pillar 03 link
        assembled = re.sub(
            r'(PILLAR 03.*?<a [^>]*href=")#(")',
            r'\1teachers-debate-institute.html\2',
            assembled,
            flags=re.DOTALL
        )
        # Replace pillar 04 link
        assembled = re.sub(
            r'(PILLAR 04.*?<a [^>]*href=")#(")',
            r'\1student-training.html\2',
            assembled,
            flags=re.DOTALL
        )
